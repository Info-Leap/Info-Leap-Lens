...
# Leave unset (or =0) for local dev â€” skips login entirely.
...
if st.sidebar.button("ðŸ” Jump to... (âŒ˜K)", use_container_width=True):
    show_jump_menu()

# â”€â”€ Quant project switcher (2026-07-27, Phase 3 of multi-project ingestion) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# Only shown once >1 project actually exists â€” a single-project install (the common case today)
# gets no extra UI. Selecting a project writes active_project_id, which get_db_path() (in
# oxdata/db_loader.py) reads for every call site that doesn't pass project_id explicitly â€” no
...
if len(_available_projects) > 1:
    ...
    # "_last_active_project_id" has ever been set), that made this fallback self-referential â€”
    ...
    _selected_project = st.sidebar.selectbox(
        "ðŸ“ Active Project", _available_projects,
    ...
    if _selected_project != _prev_project:
        # 2026-07-27: found live-testing the switch â€” every @st.cache_data-wrapped data function
        ...
        # separately expires. Clearing on every actual switch (not every rerun â€” _prev_project
...
pg = st.navigation(
    {
        "Workspace": [
            st.Page("views/dashboard.py",     title="Home",            icon="ðŸ ", default=True),
            st.Page("views/brand_health.py",  title="Brand Health",    icon="ðŸ“ˆ"),
            st.Page("views/quote_explorer.py", title="Quote Explorer", icon="ðŸ—£ï¸"),
        ],
        "Data": [
            st.Page("views/repository.py",    title="Repository",      icon="ðŸ“š"),
            st.Page("views/formula_reference.py", title="Formulas",    icon="ðŸ§ª"),
            st.Page("views/add_project.py",   title="Add Project",     icon="âž•"),
            st.Page("views/manage_projects.py", title="Manage Projects", icon="ðŸ—‚ï¸"),
            st.Page("views/settings.py",      title="Settings",        icon="âš™ï¸"),
        ],
    }
)
...
if _user:
    ...
    st.sidebar.markdown("""
        <div class="sidebar-footer">
            Press âŒ˜K anywhere to jump between screens.
        </div>
    """, unsafe_allow_html=True)
