...
it calls a real (free-tier) OpenRouter LLM â€” deselected by default; run with
`-m live`.
...
def test_akshayakalpa_full_pipeline_matches_baseline(tmp_path):
    ...
    if not api_key:
        pytest.skip("OPENROUTER_API_KEY not set â€” skipping live regression test")
    ...
    # pandas Timestamp cells â€” the real raw datafile has at least one datetime
    ...
    out_db_path = str(tmp_path / "oxdata.db")
