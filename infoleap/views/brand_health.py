...
from infoleap.analytics.brand_narrative import generate_brand_narrative
from infoleap.analytics.mapping_utils import _get_attr_themes, ATTRIBUTE_THEMES
from infoleap.utils.test_data_loader import list_test_datasets, load_test_matrix
from infoleap.utils.ui_styles import (inject_pulse_styles, sidebar_context_block,
                                    ...
                                    COLORS, CHART_LAYOUT)
from infoleap.utils.error_handlers import show_error_card
from infoleap.config.project_1 import BENCHMARKS

# data_layer: raw-Excel compute path (activated when project has raw_data.xlsx + llm_mapping_raw.json)
try:
    from infoleap.data_layer import get_project_layer, ProjectDataLayer, get_all_brands
...
import infoleap.analytics.bip_engine as _bip_mod_ref
import infoleap.analytics.can_map_engine as _ca_mod_ref
import infoleap.analytics.brand_imagery_engine as _bie_mod_ref
...
def _wb_ask(question: str, model_choice: str):
    ...
    try:
        from infoleap.db_loader import get_project_meta as _get_meta
        _pmeta = _get_meta()
...
def _get_cached_briefing(brand_name, b_data, b_n, z_data, c_nps, r_list,
    ...
    import json as _json
    from infoleap.analytics.brand_narrative import generate_executive_briefing
    try:
        b_all = _json.loads(b_all_str)
    except Exception:
        b_all = []
...
def _generate_interps_from_db(
    ...
    try:
        from infoleap.analytics.imagery_interpreter import generate_all
...
def _run_bip(product, section, pctile, zone="all", gender="all", age_band="all", city="all", top_brands=19, project_id="project_1"):
    ...
    from infoleap.analytics.can_map_engine import PRODUCT_CODES
    cats = PRODUCT_CODES.get(product, [1, 2, 3, 4, 5, 6])
...
def _render_section_13_can_map(sel_cat, zone_arg="all", gender_arg="all", age_band_arg="all", city_arg="all", sel_brand=None, project_id="project_1", attr_ids=None, awareness_stages=None):
    ...
    if attr_ids:
        ...
        from infoleap.analytics.can_map_engine import run_ca_pipeline as _ca_filtered, PRODUCT_CODES as _PC_CA
    ...
    if ca_res["status"] == "ok":
        ...
        with tabs[2]:
            from infoleap.analytics.can_map_engine import build_perceptual_map as _bpm
            _sym_view = st.radio(
        ...
        with tabs[3]:
            ...
            with asym_tabs[0]:
                from infoleap.analytics.can_map_engine import build_perceptual_map as _bpm_a
                _asym_view = st.radio(
                    "View mode", ["Aggregated (default)", "Custom (attribute filter)"],
            ...
            with asym_tabs[1]:
                from infoleap.analytics.can_map_engine import build_biplot_map as _bbm
                _bip_view = st.radio(
                    "View mode", ["Aggregated (default)", "Custom (attribute filter)"],
...
def _render_section_14_bip(sel_cat, zone_arg="all", gender_arg="all", age_band_arg="all", city_arg="all", sel_brand=None, project_id="project_1", attr_ids=None):
    ...
    try:
        from infoleap.db_loader import get_db_path as _bip_get_dbp
        ...
        if _dbp and _dbp.exists():
            ...
            from infoleap.data_layer import get_project_layer as _bip_get_lyr
    ...
    if attr_ids:
        # Driver-filtered mode: bypass cache, call engine directly with attr_ids
        from infoleap.analytics.bip_engine import BIPNormalizationEngine as _BIPFiltered
        from infoleap.analytics.can_map_engine import PRODUCT_CODES as _PC_BIP
        ...
        bip_res       = _run_bip(bip_cat, None if bip_section == "All" else bip_section, bip_pctile,
                                  ...
                                  project_id=project_id)
    if attr_ids:
        from infoleap.analytics.can_map_engine import run_ca_pipeline as _ca_bip_filt, PRODUCT_CODES as _PC_CA_BIP
...
def _driver_regression_from_layer(layer, brand, dv_kind, topbox_min, zone="all", gender="all",
    ...
    from infoleap.skills.r_bridge import run_r_stat
...
def _render_driver_regression(sel_brand, brands_list, zone="all", gender="all",
    ...
    try:
        ...
        from infoleap.db_loader import get_db_path as _drgdb
...
def render_brand_health_dashboard():
    ...
    try:
        from infoleap.db_loader import get_project_meta as _bh_get_meta
    ...
    _p_meta = {}
    try:
        from infoleap.db_loader import get_project_meta as _bh_get_meta_top
    ...
    def _tab_imagery():
        with st.container(border=True):
            ...
            _has_imagery = False
            try:
                from infoleap.db_loader import get_db_path as _gdbp
    ...
    def _tab_driver_flow():
        ...
        with st.container(border=True):
            ...
            try:
                ...
                from infoleap.db_loader import get_db_path as _df_gdb
