...
# free-tier-only cost behavior. DeepSeek R1 is chosen as the strongest reasoning
# model in the free list below (also matches the project's existing primary-model
# convention in skills/llm_client.py, which independently settled on a DeepSeek
# model after a documented comparison).
#
# 2026-08-31: every model in _FREE_MODELS now 404s (OpenRouter pulled/paywalled
# the free tier) — _FREE_MODELS below has a paid gpt-4o-mini appended as a last
# resort so this doesn't just fail outright. The "free-tier-only" framing here is
# now aspirational rather than a hard guarantee; see the note on _FREE_MODELS.
...
_FREE_MODELS = [
    ...
    # 2026-08-31: OpenRouter pulled/paywalled every :free model above (all now 404) — the
    # "no paid fallback" guarantee above stopped holding on its own, since without ANY
    # working model this call path just failed outright rather than staying free. Explicit
    # user decision (not a default): accept per-call billing on gpt-4o-mini rather than leave
    # every AI narrative on this page broken. Revisit if OpenRouter restores the free tier.
    "openai/gpt-4o-mini",
]
                    ...
                    if not _has_t:
                        ...
                        if not _pj.exists():
                            ...
                            _minimal = {
                                ...
                                "status":           "raw",
                                "description":      "",
                                # "segment" (respondent.segment) is the universal field every
                                # extraction schema produces; "brand_owned" only exists for
                                # ethnographic/brand-ownership studies and left this field showing
                                # "No brand_owned values found" for every other study_type (concept
                                # testing, attitude studies, etc) uploaded through this ZIP path.
                                "segment_key":      "segment",
                                "filter_keys":      ["segment", "city"],
                                "data_paths": {
                                    "transcripts": f"data/projects/{_proj_id_new}/transcripts",
                                    "matrices":    f"data/projects/{_proj_id_new}/matrices",
                                    "source_docs": f"data/projects/{_proj_id_new}/source_docs",
                                    "schema":      f"data/projects/{_proj_id_new}/schema/extraction_schema.json",
                                },
...
def _run_qual_extraction_pipeline_ui(proj_id: str, project_dir):
    ...
    Calls the underlying functions in-process (no subprocess) so progress can be streamed
    via st.status(), matching the staged-progress pattern used in add_project.py's
    "AI Classification Pipeline" panel.

    Safe to call on a project that already has some artifacts — each stage is skipped
    (unless force is checked) rather than re-run, so this also serves as a "resume" button
    ...
    with _c1:
        st.caption(
            f"Discussion guide: `{_dg_name}`" if _dg_name else
            "Discussion guide: not found in `source_docs/` — schema generation will proceed "
            "without it (transcript-grounded discovery only)."
        )
        st.caption(
            f"AI analysis prompt: `{_prompt_name}`" if _prompt_name else
            "AI analysis prompt: not found in `source_docs/` — schema generation will proceed "
            "without it."
        )
    with _c2:
        _force_all = st.checkbox("Force re-run all steps", key=f"_qp_force_{proj_id}",
                                  help="Re-generate schema and re-extract every transcript, "
                                       "even ones that already have output.")

    if st.button("▶ Run Full Pipeline", key=f"_qp_run_{proj_id}", type="primary"):
        from infoleap.skills import schema_generator as _sg
        from infoleap.skills import docx_to_md as _d2m
        from infoleap.skills import project_extractor as _pex_run

        _ok = True
        with st.status("Step 1/3 — Generating extraction schema…", expanded=True) as _status:
            try:
                if not _force_all and schema_out.exists() and prompt_out.exists():
                    st.write("Schema already exists — skipping (check 'Force re-run all steps' to regenerate).")
                else:
                    st.write(f"DG: {_dg_name or 'not found'} | AI Prompt: {_prompt_name or 'not found'}")
                    _sg.generate_schema(
                        proj_id,
                        dg_path=_dg_matches[0] if _dg_matches else None,
                        prompt_path=_prompt_matches[0] if _prompt_matches else None,
                    ...
                    st.write("Schema + master prompt written.")
                _status.update(label="Step 1/3 — Schema ready", state="complete")
                # Propagate the inferred project_type (concept_testing / ethnographic / ...)
                # from extraction_schema.json into project.json's study_type — without this,
                # a fresh ZIP-installed project always defaults to the ethnographic renderer
                # (hardcoded to a "brand_owned" field) regardless of what kind of study it
                # actually is, showing "No brand_owned values found" for anything else.
                try:
                    if schema_out.exists():
                        _sch = json.loads(schema_out.read_text(encoding="utf-8"))
                        _ptype = _sch.get("project_type")
                    ...
                    pass  # best-effort — worst case study_type stays unset, same as before this patch
            except SystemExit as _e:
                _ok = False
                st.error(f"Schema generation exited (code {_e.code}) — check logs / OpenRouter key.")
                _status.update(label="Step 1/3 — Schema generation failed", state="error")
            except Exception as _e:
                _ok = False
                st.error(f"Schema generation failed: {_e}")
                _status.update(label="Step 1/3 — Schema generation failed", state="error")
        ...
        if _ok:
            with st.status("Step 2/3 — Converting transcripts (.docx → .md)…", expanded=True) as _status:
                try:
                    _idx = _d2m.process_project(proj_id, force=_force_all)
                    if _idx:
                        _n_ok = sum(1 for v in _idx.values() if v.get("status") == "ok")
                        _n_skip = sum(1 for v in _idx.values() if v.get("status") == "skipped")
                        _n_err = sum(1 for v in _idx.values() if v.get("status") == "error")
                        ...
                        st.write("No new transcripts to convert (or none found).")
                    _status.update(label="Step 2/3 — Transcripts converted", state="complete")
                except SystemExit as _e:
                    _ok = False
                    st.error(f"Transcript conversion exited (code {_e.code}).")
                    _status.update(label="Step 2/3 — Transcript conversion failed", state="error")
                except Exception as _e:
                    _ok = False
                    st.error(f"Transcript conversion failed: {_e}")
                    _status.update(label="Step 2/3 — Transcript conversion failed", state="error")
        ...
        if _ok:
            with st.status("Step 3/3 — Extracting quotes (LLM pass per transcript)…", expanded=True) as _status:
                try:
                    if not index_path.exists():
                        st.error("processed_index.json still missing after step 2 — cannot extract.")
                        _status.update(label="Step 3/3 — Extraction skipped", state="error")
                    else:
                        _pex_run.run_extraction(proj_id, force=_force_all)
                        _report_path = project_dir / "extraction_report.json"
                        if _report_path.exists():
                            _rep = json.loads(_report_path.read_text(encoding="utf-8"))
                            st.write(f"Extraction complete: {_rep.get('ok', 0)} OK, "
                                     f"{_rep.get('skipped', 0)} skipped, {_rep.get('errors', 0)} errors "
                                     f"(of {_rep.get('total', 0)} transcripts).")
                        _status.update(label="Step 3/3 — Extraction complete", state="complete")
                except SystemExit as _e:
                    _ok = False
                    st.error(f"Extraction exited (code {_e.code}).")
                    _status.update(label="Step 3/3 — Extraction failed", state="error")
                except Exception as _e:
                    _ok = False
                    st.error(f"Extraction failed: {_e}")
                    _status.update(label="Step 3/3 — Extraction failed", state="error")

        # Verification + findings mirror ProjectManager.trigger_processing()'s later steps
        # (used by _render_project_setup's "Run Extraction Now" for the generic project view) —
        # best-effort, non-blocking: matrices are the real deliverable, these just enrich them.
        if _ok:
            try:
                from infoleap.skills import verify_verbatims as _vv
                with st.status("Verifying quote grounding against transcripts…", expanded=False) as _vst:
                    try:
                        _vv.run_verification(proj_id)
                        _vst.update(label="Verification complete", state="complete")
                    except SystemExit as _e:
                        _vst.update(label=f"Verification skipped (exit {_e.code})", state="error")
                    except Exception as _e:
                        _vst.update(label=f"Verification skipped: {_e}", state="error")
            except Exception:
                pass
            try:
                from infoleap.skills import findings_generator as _fg
                with st.status("Generating findings report…", expanded=False) as _fst:
                    try:
                        _fg.run_generation(proj_id)
                        _fst.update(label="Findings generated", state="complete")
                    except SystemExit as _e:
                        _fst.update(label=f"Findings skipped (exit {_e.code})", state="error")
                    except Exception as _e:
            ...
            try:
                ...
                _pm._update_registry_entry(proj_id, status="processed",
                                            last_processed=str(_date.today()))
            except Exception:
                pass

            # Sync qual outputs to Drive — schema, matrices, findings — mirroring the quant
            # pipeline's _auto_sync_to_drive in add_project.py. No-op if Drive isn't configured
            # (DriveClient._svc is None) or STORAGE_BACKEND isn't gdrive; never blocks the UI.
            try:
                from infoleap.gdrive.client import DriveClient as _DC
                _dc = _DC()
                if _dc._svc is not None:
                    with st.status("Syncing to Drive…", expanded=False) as _dst:
                        _synced = []
                        _schema_dir = project_dir / "schema"
                        _matrices_dir = project_dir / "matrices"
                        _findings_dir = project_dir / "findings"
                        if _schema_dir.exists() and _dc.upload_qual_schema(proj_id, str(_schema_dir)):
                            _synced.append("schema")
                        if _matrices_dir.exists() and _dc.upload_qual_matrices(proj_id, str(_matrices_dir)):
                            _synced.append("matrices")
                        if _findings_dir.exists() and _dc.upload_qual_findings(proj_id, str(_findings_dir)):
                            _synced.append("findings")
                        _pj_file = project_dir / "project.json"
                        if _pj_file.exists() and _dc.upload_qual_file(proj_id, str(_pj_file), "project.json"):
                            _synced.append("project.json")
                        if _synced:
                            _dst.update(label=f"☁️ Synced to Drive: {', '.join(_synced)}", state="complete")
                        else:
                            _dst.update(label="Drive sync skipped — nothing to upload", state="complete")
            except Exception:
    ...
    with st.expander(
        ...
        if _layer2_fields:
            ...
            with st.container(border=True):
                ...
                _cf_objective = st.text_area(
                    "Analysis objective for this combined field *(required)*",
                    key=f"{proj_id}_es_combine_objective",
                    placeholder=(
                        "What should the extractor look for when filling this field? "
                        "e.g. 'Capture how the respondent weighs trust barriers against "
                        "platform credibility signals — cite the specific concern and any "
                        "trust trigger they mention in the same breath.'"
                    ),
                    height=90,
                    help="This becomes the extraction instruction for every future transcript run. "
                         "Without it the model has no analytical direction for the merged concept.",
                )
                if st.button("🧬 Combine into new field", key=f"{proj_id}_es_combine_apply",
                             disabled=len(_cf_sel) < 2 or not _cf_name or not _cf_objective.strip()):
                    if _cf_name in _layer2_fields:
                        ...
                        schema.setdefault("layer2", {}).setdefault("fields", {})[_cf_name] = {
                            ...
                            "_source_objective": _cf_objective.strip(),
        ...
        if matrices_dir and matrices_dir.exists() and any(matrices_dir.glob("*_matrix.json")):
            ...
            with st.container(border=True):
                ...
                _vg_focus = st.text_input(
                    "Research focus for grouping *(optional but recommended)*",
                    key=f"{proj_id}_vg_focus",
                    placeholder=(
                        "e.g. 'Group around trust signals, adoption barriers, and return expectations' "
                        "or 'Focus on emotional vs. rational decision drivers'"
                    ),
                    help="Tells the AI what analytical lens to apply when naming and grouping themes. "
                         "Without this, it uses a generic sentiment × topic split that may not match "
                         "your study's actual research questions.",
                )
                        ...
                        if not _quotes:
                            ...
                            with st.spinner(f"Grouping {len(_quotes)} quotes..."):
                                _proposed = _vg.propose_groups(_quotes, proj.get("display_name", proj_id),
                                                                ...
                                                                research_focus=_vg_focus.strip() or None)
                            if not _proposed:
        ...
        if reviewable:
            ...
            with st.container(border=True):
                ...
                if st.button(f"✅ Finalize {n_approved} approved (Step 2 + write matrices)",
                    ...
                    # Sync updated matrices to Drive so extraction output isn't lost on
                    # Streamlit Cloud instance restart (ephemeral filesystem).
                    try:
                        from infoleap.gdrive.client import DriveClient as _DC5
                        _dc5 = _DC5()
                        if _dc5._svc is not None and matrices_dir.exists():
                            _dc5.upload_qual_matrices(proj_id, str(matrices_dir))
                    except Exception:
                        pass
                    st.cache_data.clear()
