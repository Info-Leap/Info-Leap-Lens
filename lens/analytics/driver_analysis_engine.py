...
class DriverAnalysisEngine:
    ...
    def _compute_nps_impacts(self, driver_ids: list[int], category_codes: list[int], brands: list[str], pooled: bool = False, dv_stage: str = "NPS") -> tuple[pd.Series, dict]:
        ...
        """
        try:
            from oxdata.skills.r_bridge import run_r_stat as _run_r_stat, r_available as _r_available
            _use_r = _r_available()
        except Exception:
            _use_r = False
        ...
        if self.project_id:
            try:
                from lens.data_layer import project_has_raw_layer, get_project_layer
