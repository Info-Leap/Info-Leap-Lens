...
from oxdata.skills.qual_retriever import search_qual_trees, synthesize_qual_insights
from oxdata.skills.project_manager import ProjectManager
...
def _get_top_extraction_models(n: int = 20) -> dict:
    ...
    if not paid and not _FREE_MODELS:
        from oxdata.skills.llm_client import MODEL as _default_model, FALLBACK_MODELS as _fallback_models
...
def _doc_id_for_project(proj: dict, filename: str) -> str:
    ...
    """
    from oxdata.skills import project_extractor as _pex
...
def _render_pipeline_sync_banner(proj_id: str):
    ...
    from oxdata.skills.project_extractor import pipeline_sync_status
    try:
...
def _render_extraction_studio(proj_id: str, proj: dict):
            ...
            if _scope_fields_missing:
                with st.container(border=True):
                    ...
                    if st.button("🔍 Search all missing fields for evidence",
                        ...
                        from oxdata.skills import schema_generator as _sg
        ...
        with st.container(border=True):
            ...
            if _draft_clicked and _brief_path:
                with st.spinner("Reading the brief and drafting detailed guidance…"):
                    from oxdata.skills.llm_client import call_llm_safe
                    _dg_text_for_draft = ""
                    if source_docs_dir:
    ...
    with st.expander(
            ...
            with st.container(border=True):
                ...
                with st.expander(f"🧩 {len(_layer2_fields)} discovered field(s)", expanded=_fr_pending):
                    ...
                    if st.button(_btn_label, key=f"{proj_id}_es_apply_fields"):
                        ...
                        from oxdata.skills import schema_generator as _sg2
            ...
            with st.container(border=True):
                ...
                if st.button("🧬 Combine into new field", key=f"{proj_id}_es_combine_apply",
                    ...
                    if _cf_name in _layer2_fields:
                        ...
                        schema_path.write_text(json.dumps(schema, indent=2, ensure_ascii=False), encoding="utf-8")
                        from oxdata.skills import schema_generator as _sg3
        ...
        if matrices_dir and matrices_dir.exists() and any(matrices_dir.glob("*_matrix.json")):
            ...
            with st.container(border=True):
                from oxdata.skills import verbatim_grouping as _vg
...
if _study_type == "concept_testing":
    from oxdata.views.concept_testing_renderer import render_concept_testing
...
if _study_type == "ethnographic":
    from oxdata.views.ethnographic_renderer import render_ethnographic
...
if _study_type not in ("concept_testing", "ethnographic"):  # unknown study types → generic renderer
    ...
    if _gen_m_count > 0:
        try:
            from oxdata.views.qual_generic_renderer import render_generic_project, load_ui_config
